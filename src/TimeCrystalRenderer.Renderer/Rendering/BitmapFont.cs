...
/// <summary>
/// A hardcoded 8x8 pixel bitmap font covering ASCII 32-126.
/// Each character is 8 bytes — one per row, MSB = leftmost pixel.
/// Based on the classic VGA/CP437 8x8 font (public domain).
/// </summary>
public static class BitmapFont
{
    ...
    /// <summary>
    /// Returns the 8 bytes defining a character's pixel pattern.
    /// Returns a blank pattern for characters outside the supported range.
    /// </summary>
    public static ReadOnlySpan<byte> GetGlyph(char character)
    {
        int index = character - FirstChar;
        if (index < 0 || index >= CharCount)
            return GlyphData.AsSpan(0, CharHeight); // Space (blank)

        return GlyphData.AsSpan(index * CharHeight, CharHeight);
    }

    /// <summary>
    /// Builds a font atlas texture as a single-channel byte array.
    /// Characters are laid out in a single row: 95 chars × 8px = 760px wide, 8px tall.
    /// </summary>
    public static byte[] BuildAtlas(out int atlasWidth, out int atlasHeight)
    {
        atlasWidth = CharCount * CharWidth;
        atlasHeight = CharHeight;
        var atlas = new byte[atlasWidth * atlasHeight];

        for (int charIndex = 0; charIndex < CharCount; charIndex++)
        {
            var glyph = GlyphData.AsSpan(charIndex * CharHeight, CharHeight);
            ...
            for (int row = 0; row < CharHeight; row++)
            {
                ...
                for (int col = 0; col < CharWidth; col++)
                {
                    ...
                    int x = charIndex * CharWidth + col;
                    int y = row;
                    atlas[y * atlasWidth + x] = isSet ? (byte)255 : (byte)0;
                }
            }
        }
    ...
    private static readonly byte[] GlyphData =
    {
        // 32: Space
